...
public class LuceneBioPassageCandidateFinder {
	...
	public List<PassageCandidate> extractPassages(String question,
			List<String> keyterms, Set<String> stopWords) {// ,
		...
		try {
			...
			TextFragment textFrags[] = highlighter.getBestTextFragments(
					tokenStream, text, true, 10);
			for (int j = 0; j < textFrags.length; j++) {
				TextFragment textFrag = textFrags[j];
				System.out
						.println("########################################################");
				System.out.println(textFrag.getScore()
						+ "\t"
						+ textFrag.getTextStartPos()
						+ "\t"
						+ textFrag.getTextEndPos()
						+ "\t"
						+ text.substring(textFrag.getTextStartPos(),
								textFrag.getTextEndPos()));
	...
	/*
	 * public static void main(String[] args) { LuceneBioPassageCandidateFinder
	 * passageFinder1 = new LuceneBioPassageCandidateFinder("1",
	 * "The quick brown fox jumped over the quick brown fox.");
	 * LuceneBioPassageCandidateFinder passageFinder2 = new
	 * LuceneBioPassageCandidateFinder("1",
	 * "The quick brown fox jumped over the quick brown fox."); String[]
	 * keyterms = { "quick", "jumped" }; List<PassageCandidate> windows1 =
	 * passageFinder1 .extractPassages("quick jumped");
	 * System.out.println("Windows (product scoring): " + windows1);
	 * List<PassageCandidate> windows2 = passageFinder2
	 * .extractPassages("quick"); System.out.println("Windows (sum scoring): " +
	 * windows2); }
	 */
}
